...
class Upsample3D(nn.Module):
    def __init__(self, in_channels, with_conv):
        ...
        if self.with_conv:
            self.conv = common_net.depthwise_conv(in_channels, in_channels, dim=3, kernel_size=3, stride=1, padding=1)
...
class Downsample3D(nn.Module):
    def __init__(self, in_channels, with_conv):
        ...
        if self.with_conv:
            ...
            self.conv = common_net.depthwise_conv(in_channels, in_channels, dim=3, kernel_size=3, stride=2, padding=1)
...
class ResnetBlock3D(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.conv1 = common_net.depthwise_conv(in_channels, out_channels, dim=3, kernel_size=3, stride=1)

        if temb_channels > 0:
            self.temb_proj = torch.nn.Linear(temb_channels, out_channels)
        self.norm2 = Normalize(out_channels)
        self.dropout = torch.nn.Dropout(dropout)
        self.conv2 = common_net.depthwise_conv(out_channels, out_channels, dim=3, kernel_size=3, stride=1)
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = common_net.depthwise_conv(in_channels, out_channels, dim=3, kernel_size=3, stride=1)
            else:
...
class Encoder3D(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.conv_out = torch.nn.Conv3d(block_in, 2*z_channels if double_z else z_channels,
