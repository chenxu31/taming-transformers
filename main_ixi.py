...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=True,
        default=r"/home/chenxu/datasets/ixi/h5_data",
        help="dataset dir",
    )
    parser.add_argument(
        "--modality",
    ...
    def __init__(self, data_dir, modality, debug, ckptdir, n_slices, start_epoch):
        ...
        if modality == "t1":
            if debug:
                ...
                self.val_data, _ = common_ixi.load_test_data(data_dir, "val")
        elif modality == "t2":
            if debug:
                f = h5py.File(os.path.join(data_dir, "train_t2.h5"), "r")
                self.val_data = np.array(f["data"][0:1, :, :, :])
            else:
                _, self.val_data = common_ixi.load_test_data(data_dir, "val")
        else:
            assert 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        # data
        ixi_dataset = common_ixi.Dataset(opt.data_dir, opt.modality, n_slices=config.model.params.ddconfig.in_channels, debug=opt.do_debug, data_augment=0)
        data = DataLoader(ixi_dataset, batch_size=opt.batch_size, shuffle=True, pin_memory=True, drop_last=True, num_workers=NUM_WORKERS)

        # add callback which sets up log directory
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "main_ixi.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "learning_rate_logger": {
                "target": "main_ixi.LearningRateMonitor",
                "params": {
